...
system('mkdir ./results/DCIS_Precision_Panel_KCL_OncoPrint')
...
# Load SLO Panel samples --------------------------------------------------

samples <- as.data.frame(fread('./data/Panel/DCIS_Precision_Panel_KCL/DCIS_Precision_Panel_Sloane_Samples.txt'))
...
# Load SLO Panel filtered mutations ---------------------------------------------
...
eventDataFrame_all <- eventDataFrame_mutect[,c('ExonicFunc.refGene', 'Chr', 'Start', 'End', 'Ref', 'Alt',
                                              'AF_PDCIS', 'Gene.refGene', 'patient_id', 'tissue_pathology', 'first_subseq_event',
...
for (status in c('case', 'control')){
  ...
  for (i in 1:nrow(geneMatrix)) {
    ...
    for (j in 1:ncol(geneMatrix)) {
      Ind <- which(eventDataFrame$patient_id==rownames(geneMatrix)[i] & eventDataFrame$Gene.refGene==colnames(geneMatrix)[j])
      if (length(Ind)==1) {
        geneMatrix[i,j] <- eventDataFrame$ExonicFunc.refGene[Ind]
      }
      if (length(Ind)>1) {
        if (length(unique(eventDataFrame$patient_id[Ind]))==1) {  
          ...
          geneMatrix[i,j] <- eventDataFrame$ExonicFunc.refGene[Ind[1]]
        }
      }
    }
  }
  ...
  samples[rownames(geneMatrix_bin),] <- rowSums(geneMatrix_bin)
  ...
  # oncoplot
  pdf(paste0("./results/DCIS_Precision_Panel_KCL_OncoPrint/oncoPrint_panel_", unique(eventDataFrame$case_control),".pdf"), height = 5)
...
write.csv(samples, './data/Panel/DCIS_Precision_Panel_KCL/DCIS_Precision_Panel_Sloane_Samples_v2.txt')
