...
write.table(fisherTable, './results/diff_mut_overall.csv', sep = ',', row.names = F)

#forest plot
if (length(which(fisherTable$adjPval < 0.05))>0){
  pdf('./results/diff_mut_overall.pdf')
...
for (rt in c(0, 1)){
  ...
  write.table(fisherTable, paste0('./results/diff_mut_RT_', rt, '.csv'), sep = ',', row.names = F)
  
  if (length(which(fisherTable$adjPval < 0.05))>0){
    pdf( paste0('./results/diff_mut_RT_', rt, '.pdf'))
...
for (her2 in c(0, 1)){
  ...
  write.table(fisherTable, paste0('./results/diff_mut_HER2_', her2, '.csv'), sep = ',', row.names = F)
  
  if (length(which(fisherTable$adjPval < 0.05))>0){
    pdf( paste0('./results/diff_mut_Her2_', her2, '.pdf'))
...
for (er in c(0, 1)){
  ...
  write.table(fisherTable, paste0('./results/diff_mut_ER_', er, '.csv'), sep = ',', row.names = F)
  
  if (length(which(fisherTable$adjPval < 0.05))>0){
    pdf( paste0('./results/diff_mut_ER_', er, '.pdf'))
...
write.table(fisherTable, './results/diff_mut_overall_CN.csv', sep = ',', row.names = F)

#forest plot
if (length(which(fisherTable$adjPval < 0.05))>0){
  pdf('./results/diff_mut_overall_CN.pdf')
  forestPlotFisher(fisherTable, SampleSheet, fdr=0.05)
  dev.off()
}
...
for (rt in c(0, 1)){
  ...
  write.table(fisherTable, paste0('./results/diff_mut_RT_', rt, '_CN.csv'), sep = ',', row.names = F)
  
  if (length(which(fisherTable$adjPval < 0.05))>0){
    pdf( paste0('./results/diff_mut_RT_', rt, '_CN.pdf'))
    forestPlotFisher(fisherTable, SampleSheet = SampleSheet[SampleSheet$radiotherapy == rt,], fdr=0.05)
    dev.off()
  }
}
...
for (her2 in c(0, 1)){
  ...
  write.table(fisherTable, paste0('./results/diff_mut_HER2_', her2, '_CN.csv'), sep = ',', row.names = F)
  
  if (length(which(fisherTable$adjPval < 0.05))>0){
    pdf( paste0('./results/diff_mut_Her2_', her2, '_CN.pdf'))
    forestPlotFisher(fisherTable, SampleSheet = SampleSheet[SampleSheet$her2 == her2,], fdr=0.05)
    dev.off()
  }
}
...
for (er in c(0, 1)){
  ...
  write.table(fisherTable, paste0('./results/diff_mut_ER_', er, '_CN.csv'), sep = ',', row.names = F)
  
  if (length(which(fisherTable$adjPval < 0.05))>0){
    pdf( paste0('./results/diff_mut_ER_', er, '_CN.pdf'))
    forestPlotFisher(fisherTable, SampleSheet = SampleSheet[SampleSheet$er == er,], fdr=0.05)
    dev.off()
  }
}
