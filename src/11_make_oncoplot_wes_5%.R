...
GenesPanel_ca <- GenesPanel_ca[GenesPanel_ca>3]
GenesPanel_co <- table(eventDataFrame$Hugo_Symbol[eventDataFrame$case_control=='control'])
GenesPanel_co <- GenesPanel_co[GenesPanel_co>3]
...
for (status in c('case', 'control')){
  ...
  # oncoplot
  pdf(paste0("./results/oncoPrint_wes_", status,"_5.pdf"), height = 6.5)
...
pdf('./results_per_platform/WES/PRE_SLO4673_vaf.pdf')
plot(as.numeric(eventDataFrame_mutect$t_vaf[eventDataFrame_mutect$Entrez_Gene_Id=='PRE_SLO4673']))
dev.off()
